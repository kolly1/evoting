...
<title>Post of General Secretary | NUESA 2018/2019 Election</title>
...
<div id="content">
<div id="ased">
BELOW ARE ASPIRANTS FOR THE POST OF GENERAL SECRETARY. SELECT YOUR CHOICE AND CLICK ON CAST YOUR VOTE BUTTON TO CONTINUE.
</div>
<div id="vote">
...
if(isset($_POST['submitted'])){
	$voterid = $_SESSION['vota_id'];
	 ...
	 $check = "select * from voterecords where voters_id like '$voterid' and aspirant_post ='General Secretary'";
	 $checkquery = mysqli_query($dbc,$check) or die(mysqli_error($dbc));
	 $num = mysqli_num_rows($checkquery);
if ($num == 1) 
{
$saa = "UPDATE `voterecords` SET `aspirant_id`='$o' WHERE `voters_id` ='$voterid' and aspirant_post ='General Secretary'";
	 $result2 = mysqli_query($dbc,$saa) or die(mysqli_error($dbc));
	  if ($result2 = mysqli_affected_rows($dbc) == 1)
{
	 header('Location:castvote4.php');
...
{
	   $aaa = "INSERT INTO voterecords VALUES('General Secretary','$o','$voterid')";
...
echo '</form>'; 
/**echo '<form action="castvote3.php" method="post">';
echo '<span id="spryradio1">';
...
/**
	...
	echo '<input type="hidden" name="submitted" value="TRUE" />';
 echo '<input type="hidden" name="id" value="'.$id.'" />';
echo '<label><input type="submit" name="SubmitButton" id="SubmitButton"  value="CAST YOUR VOTE"  onclick="DisableButton(this);" /></label>';
echo '</form>';
**/
?>
