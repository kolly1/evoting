...
<title>Post of P R O | NUESA 2019 Election</title>
...
<div id="content">
<div id="ased">
BELOW ARE ASPIRANTS FOR THE POST OF P R O. SELECT YOUR CHOICE AND CLICK ON CAST YOUR VOTE BUTTON TO CONTINUE.
</div>
<div id="vote">
<?php
if(isset($_POST['submitted'])){
	$voterid = $_SESSION['vota_id'];
	 ...
	 $check = "select * from voterecords where voters_id like '$voterid' and aspirant_post ='PRO'";
	 $checkquery = mysqli_query($dbc,$check) or die(mysqli_error($dbc));
	 $num = mysqli_num_rows($checkquery);
if ($num == 1) 
{
$saa = "UPDATE `voterecords` SET `aspirant_id`='$o' WHERE `voters_id` ='$voterid' and aspirant_post ='PRO'";
	 $result2 = mysqli_query($dbc,$saa) or die(mysqli_error($dbc));
	  if ($result2 = mysqli_affected_rows($dbc) == 1)
{
	 header('Location:castvote5.php');
...
{
	   $aaa = "INSERT INTO voterecords VALUES('PRO','$o','$voterid')";
...
echo '</form>'; 
/**
echo '<form action="castvote4.php" method="post">';
echo '<span id="spryradio1">';
