...
<title>Post Of Financial Secretary | NUESA 2019 ELECTION</title>
...
<span id="spryradio1">-->
<div id="ased">
BELOW IS THE ASPIRANT FOR THE POST OF SOCIAL DIRECTOR. SELECT YOUR CHOICE AND CLICK ON CAST YOUR VOTE BUTTON TO CONTINUE.
</div>
<div id="vote">
<?php 
if(isset($_POST['submitted'])){
	$voterid = $_SESSION['vota_id'];
	 ...
	 $check = "select * from voterecords where voters_id like '$voterid' and aspirant_post ='Social Director'";
	 $checkquery = mysqli_query($dbc,$check) or die(mysqli_error($dbc));
	 $num = mysqli_num_rows($checkquery);
if ($num == 1) 
{
$saa = "UPDATE `voterecords` SET `aspirant_id`='$o' WHERE `voters_id` ='$voterid' and aspirant_post ='Social Director'";
	 $result2 = mysqli_query($dbc,$saa) or die(mysqli_error($dbc));
	  if ($result2 = mysqli_affected_rows($dbc) == 1)
{
	 header('Location:castvote7.php');
...
{
	   $aaa = "INSERT INTO voterecords VALUES('Social Director','$o','$voterid')";
