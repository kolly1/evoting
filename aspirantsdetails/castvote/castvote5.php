...
<title>Post of Vice President (Female) | NUESA 2019 Election</title>
...
<div id="content">
<div id="ased">
BELOW ARE ASPIRANTS FOR THE POST OF Vice President (Female). SELECT YOUR CHOICE AND CLICK ON CAST YOUR VOTE BUTTON TO CONTINUE.
</div>
<div id="vote">
<?php 
if(isset($_POST['submitted'])){
	$voterid = $_SESSION['vota_id'];
	 ...
	 $check = "select * from voterecords where voters_id like '$voterid' and aspirant_post ='Vice President(F)'";
	 $checkquery = mysqli_query($dbc,$check) or die(mysqli_error($dbc));
	 $num = mysqli_num_rows($checkquery);
if ($num == 1) 
{
$saa = "UPDATE `voterecords` SET `aspirant_id`='$o' WHERE `voters_id` ='$voterid' and aspirant_post ='Vice President(F)'";
	 $result2 = mysqli_query($dbc,$saa) or die(mysqli_error($dbc));
	  if ($result2 = mysqli_affected_rows($dbc) == 1)
{
	 header('Location:castvote6.php');
...
{
	   $aaa = "INSERT INTO voterecords VALUES('Vice President(F)','$o','$voterid')";
...
echo '</form>';
/**echo '<form action="castvote5.php" method="post">';
echo '<span id="spryradio1">';
...
<div id="footer">
<img src="../images/foota.png" width="1050" height="70" /></div>
